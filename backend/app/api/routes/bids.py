...
def repository_for(session: Session) -> BidRepository | ExternalBidRepository:
    if get_settings().bid_database_url:
        return ExternalBidRepository(session)
    return BidRepository(session)


@router.get("/bids/{bid_id}", response_model=BidRecord, response_model_by_alias=True)
def get_bid(bid_id: str, session: Session = Depends(get_bid_session)) -> BidRecord:
    bid = repository_for(session).get(bid_id)
    if bid is None:
...
def check_eligibility(bid_id: str, session: Session = Depends(get_bid_session)) -> dict:
    bid = repository_for(session).get(bid_id)
    if bid is None:
        raise HTTPException(status_code=404, detail="입찰공고를 찾을 수 없습니다.")
